    ...
    try:
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            for r_idx in range(num_data_rows):
                ...
                # Col 3: Recherche LinkedIn (references Dénomination - Enseigne in col B of ENTREPRISES sheet)
                ws_entreprises.cell(
                    row=excel_row,
                    column=3,
                ...
                # Col 5: Recherche Indeed (references Dénomination - Enseigne in ENTREPRISES sheet)
                ws_entreprises.cell(
                    row=excel_row,
                    column=5,
...
def get_erm_data_for_saving() -> dict:
    ...
    and returns a dictionary formatted for JSON saving. (Currently unused)
    """
    # print(f"{dt.datetime.now()} - DEBUG - get_erm_data_for_saving called.")
    ...
    # print(f"{dt.datetime.now()} - DEBUG - Retrieved from session state: {len(df_e)} entreprises, {len(df_c)} contacts, {len(df_a)} actions.")
    ...
    if "Date Action" in df_a_cleaned.columns:
        ...
        # print(f"{dt.datetime.now()} - DEBUG - Cleaned 'Date Action' column for JSON serialization.")
    ...
    # print(f"{dt.datetime.now()} - DEBUG - ERM data converted to dict. Ready for saving.")
    return erm_data
